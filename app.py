...
# Configuración de la página (Estilo moderno y expandido)
st.set_page_config(
...
# --- 2. PERFILES DE DISTRIBUCIÓN TEMPORAL (HORAS Y DÍAS) ---
horas_eje = [f"{h:02d}:00" for h in range(24)]

# Matriz de carga horaria real extraída de las métricas de los reportes
perfiles_horarios = {
...
# Distribución semanal (Estructura de uso estrictamente laboral reflejada en los PDF)
dias_semana = ["Lunes", "Martes", "Miércoles", "Jueves", "Viernes", "Sábado", "Domingo"]
porcentajes_semanales = [0.21, 0.22, 0.22, 0.20, 0.13, 0.015, 0.005]

# --- 3. DATOS CUALITATIVOS (FOCOS DE DEMANDA) ---
insights_mes = {
    "Octubre 2025": {
    ...
    "Febrero 2026": {
        ...
        "foco": "Foco crítico en el cierre del ejercicio fiscal: presentation de la DDJJ anual (CM03) en SIFERE Web. Se detectaron fuertes señales de alerta e inconvenientes técnicos externos (la palabra 'error' escaló al 4° lugar general).",
...
if periodo_seleccionado == "Total Histórico Consolidado":
    ...
    k3.metric("Contención Automatizada Promedio", f"{100 - df['Tasa_Conversion'].mean():.2f}%", "Eficiencia estructural de menús")
    
    # Gráfico 1: Barras + Línea de conversión
    fig_hist = go.Figure()
    ...
    # Gráfico de Tendencia de Rebote Técnico
    st.subheader("2. Análisis del Fenómeno de Rebote Incidental")
    df_rebote = df[df["Tasa_Rebote"] > 0]
    fig_rebote = px.line(
        df_rebote, x="Mes", y="Tasa_Rebote", 
        text="Tasa_Rebote", title="Evolución del Rebote por Saturación Externa del Portal Principal (Meses Medidos)",
        labels={"Tasa_Rebote": "Tasa de Rebote (%)"},
...
# --- LÓGICA VISTA B: CORTE MENSUAL ESPECÍFICO ---
    ...
    # MÓDULO CORREGIDO: 6 métricas dinámicas completas para el mes seleccionado
    ...
    c5.metric("Tasa de Conversión", f"{datos_mes['Tasa_Conversion']:.2f}%", "Eficiencia del mes")
    c6.metric("Contención Automatizada", f"{100 - datos_mes['Tasa_Conversion']:.2f}%", "Absorción de menús")
    ...
    with col_der:
        st.markdown("### 📊 Palabras Clave Dominantes (Relevancia)")
        ...
        fig_kw.update_layout(showlegend=False, coloraxis_showscale=False, height=280)
        st.plotly_chart(fig_kw, use_container_width=True)
        
    # Asignación de vectores específicos del mes seleccionado
...
with col_graf1:
    ...
    fig_horas = px.area(
        df_horas, x="Hora", y="Sesiones",
        title=f"Distribución Horaria de Consultas ({periodo_seleccionado})",
        labels={"Sesiones": "Cantidad de Sesiones"},
        template="plotly_dark"
    )
    fig_horas.update_traces(line_color="#2563EB", fillcolor="rgba(37, 99, 235, 0.2)")
    fig_horas.update_layout(height=380)
    st.plotly_chart(fig_horas, use_container_width=True)

with col_graf2:
    df_dias = pd.DataFrame({"Día": dias_semana, "Sesiones": vector_dias_total})
    fig_dias = px.bar(
        df_dias, x="Día", y="Sesiones",
        title=f"Distribución por Día de la Semana ({periodo_seleccionado})",
        labels={"Sesiones": "Cantidad de Sesiones"},
        text="Sesiones",
        template="plotly_dark"
    )
    fig_dias.update_traces(marker_color="#10B981", textposition="outside")
    fig_dias.update_layout(height=380)
